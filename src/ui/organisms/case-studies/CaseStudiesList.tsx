...
const CaseStudiesList: React.FC<CaseStudiesListProps> = ({ caseStudies, filters }) => {
	...
	return (
		<div>
			...
			<div className="mt-12 grid w-full grid-cols-1 md:grid-cols-3 gap-4">
				{filteredItems.map((item, index) => (
					...
					>
						<div className="absolute bottom-4 left-4 h-4 w-4 bg-white">
							<div className="w-fit -translate-y-8 -translate-x-4 whitespace-nowrap border border-white bg-black p-4 text-white opacity-0 transition-all group-hover:scale-1 group-hover:translate-x-4 group-hover:-translate-y-16 group-hover:opacity-100">
								{item.name}
							</div>
						</div>
					</Link>
