...
const CtaSection: React.FC<CtaSectionProps> = ({ title, text, image }) => {
	return (
		<section className="relative z-10 mb-[30rem] flex justify-center px-4">
			<div
				className="xxl:max-w-[2200px] relative z-10 mx-auto flex min-h-[471px] w-full max-w-[1700px] flex-col items-start bg-basicDark bg-cover bg-center px-6 py-24 text-white md:px-32"
				style={{ backgroundImage: `url(${image})` }}
			>
				<h2 className="text-medium mb-4 max-w-[40rem] text-left text-[2rem] md:text-[3.625rem] md:leading-[72.5px]">
					{title || "Gotowy by zrealizować swój projekt z nami?"}
				</h2>
				<p className="text-medium test-[1.313rem] mb-12 text-lg text-lightGrey">
					{" "}
